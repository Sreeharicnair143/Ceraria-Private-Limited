...
const missingContent = `        <!-- Slide 1 -->
  ...
  <script>
    ...
    // 4. Init Swiper (Glance)
    new Swiper('.glance-swiper', {
      loop: true,
      effect: 'slide',
`;

const html = fs.readFileSync('public/index.html', 'utf8');
const searchStr = /([ \t]+)autoplay:\s*\{\s*delay:\s*3000\s*\},[\r\n\s]+slidesPerView:\s*1[\r\n\s]+\}\);/;
const match = html.match(searchStr);

if(!match) {
  console.log('Search string not found using regex.');
} else {
  // match[0] is the matched text. We want to prepend missingContent
  const newHtml = html.replace(searchStr, missingContent + match[0]);
